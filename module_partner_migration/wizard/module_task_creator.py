...
class ModuleInformation(models.TransientModel):
    ...
    def _get_default_partner(self):
        ...
        if len(partners) > 1:
            raise exceptions.UserError('You should create task for one partner at a time')
        return partners

    project_id = fields.Many2one('project.project', required=True)
    partner_id = fields.Many2one('res.partner', required=True, default=_get_default_partner)
